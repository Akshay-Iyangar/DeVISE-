...
def run_cnn(  arch_params,
    ...
    if data_params["type"] == 'mat':
        ...
        train_data_x, train_data_y = load_data_mat(dataset, batch = 1 , type_set = 'train')             
        test_data_x, test_data_y = load_data_mat(dataset, batch = 1 , type_set = 'test')      # Load dataset for first epoch.
        valid_data_x, valid_data_y = load_data_mat(dataset, batch = 1 , type_set = 'valid')    # Load dataset for first epoch.
        ...
        train_set_x = theano.shared(numpy.asarray(train_data_x, dtype=theano.config.floatX), borrow=False)
        train_set_y = theano.shared(numpy.asarray(train_data_y, dtype='int32'), borrow=False)
        ...
        test_set_x = theano.shared(numpy.asarray(test_data_x, dtype=theano.config.floatX), borrow=False)
        test_set_y = theano.shared(numpy.asarray(test_data_y, dtype='int32'), borrow=False) 
        ...
        valid_set_x = theano.shared(numpy.asarray(valid_data_x, dtype=theano.config.floatX), borrow=False)
        valid_set_y = theano.shared(numpy.asarray(valid_data_y, dtype='int32'), borrow=False)
        ...
        for batch in xrange (batches2train):
            ...
            if multi_load is True:
                ...
                if data_params["type"] == 'mat':
                    ...
                    train_data_x, train_data_y = load_data_mat(dataset, batch = batch + 1 , type_set = 'train')             
                ...
                #pdb.set_trace()                   
                train_set_x.set_value(train_data_x ,borrow = False)
                train_set_y.set_value(train_data_y ,borrow = False)
            ...
            if multi_load is True:
                ...
                for batch in xrange ( batches2validate ):
                    if data_params["type"] == 'mat':
                        ...
                        valid_data_x, valid_data_y = load_data_mat(dataset, batch = batch + 1 , type_set = 'valid')             
                        ...
                        if dataset == 'caltech101':
                            ...
                            # Do not use svm_flag for caltech 101                    
                    valid_set_x.set_value(valid_data_x,borrow = False)
                    valid_set_y.set_value(valid_data_y,borrow = False)
    ...
    if multi_load is False:
        ...
        for batch in xrange(batches2test):
            ...
            if data_params["type"] == 'mat':
                ...
                test_data_x, test_data_y = load_data_mat(dataset, batch = batch + 1 , type_set = 'test')             
            ...
            test_set_x.set_value(test_data_x,borrow = False)
            test_set_y.set_value(test_data_y,borrow = False)
    ...
    f = gzip.open(network_save_name, 'wb')
    for obj in [params, data_params, arch_params, optimization_params]:
...
if __name__ == '__main__':
    ...
    filename_params = { 
                        "results_file_name"     : "../results_mnist_rotated_bg.txt",        # Files that will be saved down on completion Can be used by the parse.m file
                        "error_file_name"       : "../error_mnist_rotated_bg.txt",
                        "cost_file_name"        : "../cost_mnist_rotated_bg.txt",
                        "confusion_file_name"   : "../confusion_mnist_rotated_bg.txt",
                        "network_save_name"     : "../mnist_rotated_bg1.pkl.gz "
    ...
    arch_params = {
                    ...
                    "n_epochs"                          : 500,                      # Total Number of epochs to run before completion (no premature completion)
                    ...
                    "cnn_activations"                   : [ ReLU, ReLU, ReLU, ReLU, ReLU],           # Activations for CNN layers Options: ReLU,  
                    ...
                    "nkerns"                            : [ 20 , 50, 50, 50, 64],               # Number of feature maps at each CNN layer
                    "outs"                              : 10,                       # Number of output nodes ( must equal number of classes)
                    "filter_size"                       : [  5 , 4, 3, 2, 2],                # Receptive field of each CNN layer
                    "pooling_size"                      : [  2 , 2, 1, 1, 1],                # Pooling field of each CNN layer
